...
async function saveUsuario(){
  const idPizza = document.querySelector('#id-usuario').value;
  const variedad = document.querySelector('#nombre').value;
  const ingredientes = document.querySelector('#apellido').value;
  const tamanio = document.querySelector('#correo').value;
  const precio_salon = document.querySelector('#telefono').value;
  ...
  // Crea un objeto con los datos de usuario
  const usuarioData = {
  ...
  if(idUsuario!==""){
    result = await fetchData(`${BASEURL}/api/usuarios/${idCorreo}`, 'PUT', usuarioData);
  }else{
...
async function showUsuarios(){
  ...
  usuarios.forEach((usuario,index) => {
    let tr = `<tr>
                  ...
                  <td>
                      <button class="enviar" id="editar" onclick='updateUsuario(${pizza.id_usuario})'><i class="fa fa-pencil" > Actualizar</button></i>
                      <button class="enviar" id="borrar" onclick='deleteUsuario(${pizza.id_usuario})'><i class="fa fa-trash" > Eliminar</button></i>
                  </td>
                </tr>`;
...
async function updateUsuario(id){
  ...
  nombre.value = response.variedad;
  apellido.value = response.ingredientes;
  correo.value = response.tamanio;
  telefono.value = response.precio_salon;
}
