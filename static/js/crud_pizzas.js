...
async function savePizza(){
  ...
  const precio_salón = document.querySelector('#precio_salón').value;
  const precio_delivery = document.querySelector('#precio_delivery').value;

  //VALIDACION DE FORMULARIO
  if (!variedad || !ingredientes || !tamaño|| !precio_salón || !precio_delivery) {
    Swal.fire({
        title: 'Error!',
  ...
  if(idPizza!==""){
    ...
    result = await fetchData(`${BASEURL}/api/pizza/`, 'POST', pizzaData);
  }
...
async function updatePizza(id){
  ...
  const precio_salón = document.querySelector('#precio_salón');
  ...
  tamaño.value = response.tamaño;
  precio_salón.value = response.precio_salón;
  precio_delivery = response.precio_delivery;
}
